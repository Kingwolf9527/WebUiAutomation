...
class TestTpshopLogin(object):
    ...
    def test_tpshop_login(self, account_info, get_driver):
        ...
        dd.tpshop_login(**account_info)
        assert dd.is_displayed(account_info['login_base']['exchanges']) == True
        logger.info(f"-------- 当前页面的‘交易中心’元素是否显示：{dd.is_displayed(account_info['login_base']['exchanges'])} --------")
        assert dd.get_current_url() == account_info['login_base']['login_success_url']
